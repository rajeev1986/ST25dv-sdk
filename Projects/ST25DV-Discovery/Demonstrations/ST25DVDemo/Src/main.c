...
#include "scope.h"
#include "../../../Utilities/Fonts/font36.c"
...
#ifdef INCLUDE_VALID_LIB
void BoardCheck(void);
#endif
/* Public variables ---------------------------------------------------------*/
uint8_t ST25DV_pwm = 0;
...

{
  ...
  /* Detect button to unhide pwm demo */
  if( BSP_PB_GetState(BUTTON_USER) == GPIO_PIN_SET )
  {
    tickstart = HAL_GetTick( );
    while( (BSP_PB_GetState(BUTTON_USER) == GPIO_PIN_SET) && ((HAL_GetTick( ) - tickstart) < 1000) );
...
{
  Menu_DisplayPicture( 0, 0, cStLogo);
  HAL_Delay( 1000 );
