...
void Timer_Process_Q()
{
	...
	{
		TIMER_MESG_DBG(profiledbgfile,"Timer_Process_Q");
...
tBleStatus Timer_Init(void)
{
    ...
    TIMER_MESG_DBG(profiledbgfile,"Timer_Init\n");
 ...
 */ 
tBleStatus Timer_Start(uint32_t expiryTime,
					   ...
					   tTimerID *timerID)
{
	...
	TIMER_MESG_DBG(profiledbgfile,"Timer_Start %lu\n",expiryTime);
    if ((expiryTime == 0) || 
		(timercb == NULL))
    {
...
tBleStatus Timer_Stop(tTimerID timerID)
{
	...
	TIMER_MESG_DBG(profiledbgfile,"Timer_Stop\n");
...
uint32_t Calculate_Timer_Expiry(uint32_t msTimeout)
{
  ...
  TIMER_MESG_DBG(profiledbgfile,"subTimeout %lu expTime %lu\n",subTimeout,expTime);
  return expTime;
}

void Timer_ExpiryCallback()
{
	/* a timer has expired */
