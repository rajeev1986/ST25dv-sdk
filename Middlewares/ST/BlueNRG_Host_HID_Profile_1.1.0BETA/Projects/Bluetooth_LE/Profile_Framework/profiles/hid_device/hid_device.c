...
#include <ble_hid_profile_debug.h>
                          ...
                          BLE_CALLBACK_FUNCTION_TYPE hidDeviceCallbackFunc)
{
  ...
  for(indx = 0;indx < numOfHIDServices;indx++)
  {
     ...
     */
    hidDevice.hidService[indx].protocolMode = REPORT_PROTOCOL_MODE;
  ...
  hidDevice.mainState = HID_DEVICE_STATE_WAITING_BLE_INIT;
  hidDevice.subState = HID_DEVICE_STATE_WAITING_BLE_INIT;
...
tBleStatus HidDevice_Make_Discoverable(uint8_t useBoundedDeviceList)
{
  ...
  if ((hidDevice.mainState == HID_DEVICE_STATE_ADVERTIZE)  &&  
      (hidDevice.subState == HID_DEVICE_STATE_ADVERTISABLE_IDLE))
  {
    ...
    if ((useBoundedDeviceList) && (gBLEProfileContext.bleSecurityParam.bonding_mode))
    {
      ...
      if (retval == BLE_STATUS_SUCCESS)
      {
        ...
        Timer_Start (10, HidDevice_Advertise_Period_Timeout_Handler,&hidDevice.timerID);
        /* change profile's sub states */
        hidDevice.subState = HID_DEVICE_STATE_ADVERTISING_FOR_WHITE_LIST;
    ...
    else
    {
      ...
      hidDevice.subState = HID_DEVICE_STATE_FAST_DISCOVERABLE_DO_START;
    }
  }
  else
  {
                                         ...
                                         uint8_t * ipReportValue)
{
  ...
  if ((hidDevice.mainState == HID_DEVICE_STATE_CONNECTED) &&
     (hidDevice.subState == HID_DEVICE_STATE_CONNECTED_IDLE) &&
                                          ...
                                          uint8_t batteryLevel)
{
  ...
  if((hidDevice.mainState > HID_DEVICE_STATE_INITIALIZED) && 
     (hidDevice.subState != HID_DEVICE_STATE_CONNECTED_BUSY))
  {
    ...
    case EVT_DISCONN_COMPLETE:
    {
      if (hidDevice.mainState >= HID_DEVICE_STATE_INITIALIZED)
      {
        ...
        if (cc->status == BLE_STATUS_SUCCESS)
        {
            ...
            hidDevice.mainState = HID_DEVICE_STATE_ADVERTIZE;
            hidDevice.subState = HID_DEVICE_STATE_ADVERTISABLE_IDLE;
        }
      }
      ...
      switch(evt->subevent)
      {
        case EVT_LE_CONN_COMPLETE:
        {
          ...
          if (cc->status == BLE_STATUS_SUCCESS)
          {
            ...
            hidDevice.subState = HID_DEVICE_STATE_PROCESSING_PAIRING;
          }
          else
          {
    ...
    case EVT_VENDOR:
    {
      ...
      switch(blue_evt->ecode)
      {
        case EVT_BLUE_GAP_LIMITED_DISCOVERABLE:
        {
          ...
          if ((hidDevice.mainState == HID_DEVICE_STATE_ADVERTIZE) && 
              ((hidDevice.subState == HID_DEVICE_STATE_FAST_DISCOVERABLE) ||
               (hidDevice.subState == HID_DEVICE_STATE_LOW_POWER_DISCOVERABLE)))
          {
            /* Put the main profile in connectable idle state */
            BLE_Profile_Change_Advertise_Sub_State(&hidDevice.HIDtoBLEInf, 
                                                   BLE_PROFILE_STATE_CONNECTABLE_IDLE);
                                                                                             
            if(hidDevice.subState == HID_DEVICE_STATE_LOW_POWER_DISCOVERABLE)
            {
        ...
        case EVT_BLUE_GATT_ATTRIBUTE_MODIFIED:
        {
          ...
          evt_gatt_attr_modified_IDB05A1 * attr_modified = (void*)blue_evt->data;
                  
          attrHandle = attr_modified->attr_handle;
          attValue = attr_modified->att_data[0]; 
...
void HidDevice_Advertise_Period_Timeout_Handler (void)
{
  tProfileState nextState = hidDevice.subState;
  HID_DEVICE_MESG_DBG ( profiledbgfile, "Advertising Period Timedout \n");
  if (hidDevice.mainState == HID_DEVICE_STATE_ADVERTIZE)
  {
    switch (hidDevice.subState)
    {
    ...
    if (nextState != hidDevice.subState)
    {
      hidDevice.subState = nextState;
